...
const mediaPlayer = function(t, config) {
  ...
  t.player = {
    ...
    // Switch the current track index according to the mode
    mode: function() {
    ...
    // Directly set the current track index
    ...
    // Update source to current track index
    init: function() {
  ...
  var init = function(config) {
    ...
    // Initialize buttons, controls, and click events
    buttons.create()

    // Initialize audio or video
    source = t.createChild(t.player.options.type, events);
    // Initialize playlist, preview, control buttons, etc.
    info.create();
