var sketch = function (p){
    p.isLoaded = false;
    p.glitch;
    p.imgSrc = '/images/mokou_holding_feces.png';
    p.setup = function (){
        let bodymd = document.getElementsByClassName('body md')[0]
        bodymd.id = 'bodymd'
        p.background(0);
        let cnv = p.createCanvas(378, 378 );
        cnv.parent('bodymd')
        p.loadImage(p.imgSrc, function(img) {
            p.glitch = new clitch(img);
            p.isLoaded = true;
        });
    };
    p.draw = function (){
        ...
        p.fill(255, 255, 255);
        p.textSize(14);
        p.text('FPS: ' + p.floor(p.frameRate()), 20, 30);
    }
}
...
// var isLoaded = false;
// var glitch;
// var imgSrc = '/images/mokou_holding_feces.png';
// function setup() {
//     let bodymd = document.getElementsByClassName('body md')[0]
//     bodymd.id = 'bodymd'
//     background(0);
//     let cnv = createCanvas(378, 378 );
//     cnv.parent('bodymd')
//     loadImage(imgSrc, function(img) {
//         glitch = new clitch(img);
//         isLoaded = true;
//     });
// }
...
var clitch = class {
    ...
    shiftLine(srcImg) {
        ...
        rangeMin = p5.floor(p5.random(0, rangeH));
        rangeMax = rangeMin + p5.floor(p5.random(1, rangeH - rangeMin));
        offsetX = this.channelLen * p5.floor(p5.random(-40, 40));
    ...
    shiftRGB(srcImg) {
        ...
        range = 16;
        destPixels = new Uint8ClampedArray(srcImg.pixels);
        randR = (p5.floor(p5.random(-range, range)) * srcImg.width + p5.floor(p5.random(-range, range))) * this.channelLen;
        randG = (p5.floor(p5.random(-range, range)) * srcImg.width + p5.floor(p5.random(-range, range))) * this.channelLen;
        randB = (p5.floor(p5.random(-range, range)) * srcImg.width + p5.floor(p5.random(-range, range))) * this.channelLen;
    ...
    getRandomRectImg(srcImg) {
        ...
        let rectH;
        let destImg;
        startX = p5.floor(p5.random(0, srcImg.width - 30));
        startY = p5.floor(p5.random(0, srcImg.height - 50));
        rectW = p5.floor(p5.random(30, srcImg.width - startX));
        rectH = p5.floor(p5.random(1, 50));
